        ...
        public static void DrawWorld(World world)
        {
            ...
            for (int x = 0; x < world.ChunksInRow; x++)
            {
                for (int y = 0; y < world.ChunksInRow; y++)
                {
                    ...
                    GameMain.SpriteBatch.Draw(c.RenderTarget, new Vector2(mx, my), Color.White);
                    GameMain.SpriteBatch.Draw(Textures.Textures2D.TestBuilding, new Vector2((Chunk.CHUNK_SIZE * 32) + mx, my - 81), Color.White);
                    GameMain.LastDrawedChunksCount++;
                }
            }
            GameMain.SpriteBatch.GraphicsDevice.
        }

        public static void DrawConsole()
        {
            GameMain.SpriteBatch.FillRectangle(0, 0, GameMain.SpriteBatch.GraphicsDevice.Viewport.Width, 280, Color.FromNonPremultiplied(0,0,0, 80));
            GameMain.SpriteBatch.FillRectangle(0, 280, GameMain.SpriteBatch.GraphicsDevice.Viewport.Width, 20, Color.FromNonPremultiplied(0, 0, 0, 120));
        }
    }
}
